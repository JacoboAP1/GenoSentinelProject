...
export class TumorTypesController {
    ...
    @ApiResponse({ status: 201, description: 'Tipo de tumor creado exitosamente', type: TumorTypeDtoOut })
    @ApiResponse({ status: 400, description: 'Datos inválidos o faltantes' })
    @ApiResponse({ status: 409, description: 'Ya existe un tipo de tumor con ese nombre' })
    async create(@Body() createTumorTypeDto: CreateTumorTypeDto): Promise<TumorTypeDtoOut> {
    ...
    @ApiResponse({ status: 200, description: 'Catálogo obtenido exitosamente', type: [TumorTypeDtoOut] })
    ...
    @ApiResponse({ status: 200, description: 'Búsqueda completada exitosamente', type: [TumorTypeDtoOut] })
    async searchBySystem(@Query('system') system: string): Promise<TumorTypeDtoOut[]> {
    ...
    @ApiResponse({ status: 200, description: 'Tipo de tumor encontrado', type: TumorTypeDtoOut })
    ...
    @ApiParam({ name: 'id', description: 'ID del tipo de tumor', example: 1 })
    @ApiResponse({ status: 200, description: 'Tipo de tumor actualizado exitosamente', type: TumorTypeDtoOut })
    @ApiResponse({ status: 404, description: 'Tipo de tumor no encontrado' })
    @ApiResponse({ status: 400, description: 'Datos inválidos' })
    @ApiResponse({ status: 409, description: 'Ya existe un tipo de tumor con ese nombre' })
    async update(@Param('id') id: number, @Body() updateTumorTypeDto: UpdateTumorTypeDto): Promise<TumorTypeDtoOut> {
