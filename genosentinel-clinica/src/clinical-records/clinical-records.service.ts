...
export class ClinicalRecordsService {
   ...
   */
  async create(
    createDto: CreateClinicalRecordDtoIn,
    ...
    if (!patient) {
      throw new NotFoundException(
        `Paciente con ID ${createDto.patientId} no encontrado`,
      );
    }
    ...
    if (!tumorType) {
      throw new NotFoundException(
        `Tipo de tumor con ID ${createDto.tumorTypeId} no encontrado`,
      );
    }
  ...
  async findByTumorType(tumorTypeId: number): Promise<ClinicalRecordDtoOut[]> {
    ...
    if (!tumorType) {
      throw new NotFoundException(
        `Tipo de tumor con ID ${tumorTypeId} no encontrado`,
      );
    }
    ...
    if (records.length === 0) {
      throw new NotFoundException(
        `No hay historias clínicas registradas para el tipo de tumor con ID ${tumorTypeId}`,
      );
    }
  ...
  async update(
    id: number,
    updateDto: UpdateClinicalRecordDtoIn,
  ): Promise<ClinicalRecordDtoOut> {
    const record = await this.clinicalRecordRepository.findOne({
      where: { id },
      relations: ['patient', 'tumorType'],
    ...
    if (updateDto.patientId) {
      ...
      if (!patient) {
        throw new NotFoundException(
          `Paciente con ID ${updateDto.patientId} no encontrado`,
        );
      }
    ...
    if (updateDto.tumorTypeId) {
      ...
      if (!tumorType) {
        throw new NotFoundException(
          `Tipo de tumor con ID ${updateDto.tumorTypeId} no encontrado`,
        );
      }
    ...
    if (updateDto.treatmentProtocol !== undefined)
      record.treatmentProtocol = updateDto.treatmentProtocol;
